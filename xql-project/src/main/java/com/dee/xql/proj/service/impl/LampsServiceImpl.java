	...
	public boolean addLampsData(String path, Long svnLastVersion) {
		try {
			...
			new Thread(()->{
				...
				String res = lampsMapper.generateContract(DigestUtils.md5DigestAsHex(m_fileName.getBytes()), m_svnLastVersion);
				log.info("lampsMapper.generateContract res: "+res);
			}).start(); ;
			return true;
		} catch (Exception e) {
			e.printStackTrace();
...
//			}
	...
	@Override
	public void uptLampsResource() {
		SAPGoods[] goods = this.lampsMapper.findGoodsListByType(0);
		if (goods == null || goods.length <= 0) {
			return;
		}
		this.lampsGoodsChanges(goods);
		List<SAPGoods> sapgs = new ArrayList<SAPGoods>();
		for (SAPGoods good : goods) {
			good.setType(1);
			sapgs.add(good);
		}
		this.lampsMapper.uptGoodsType(sapgs);
	}
