	...
	public ProjTaskWork projTaskWork(Task t, String fileName, String projectId, String strProjectId, Long summaryId,
		...
		for (int j = 0; j < ras.size(); j++) {
			...
			if (ra.getResource() != null) {
				if (ra.getResource().getType().getValue() == 1) {
					if (ra.getResource().getCode() != null && !ra.getResource().getCode().equals("")) {
...
//			pAllot.setPercentWorkComplete(ra.getPercentageWorkComplete().doubleValue());
	...
	public List<ResourceItem> resourceItems(ProjectFile pFile, Long summaryId, Long svnVersion) {
		...
		for (int i = 0; i < resources.size(); i++) {
			...
			rItem.setResOriginName(r.getText(1));
			rItem.setWorkGroupCode(r.getText(2));
			rItem.setNameVerify(r.getText(3));
			rItem.setVerifyAlert(r.getText(4));
			rItem.setMainPart(r.getText(5));
			rItem.setDeptName(r.getText(6));
			rItem.setJoinCode(r.getText(7));
			rItem.setLabelType(r.getText(10));
			rItem.setLabelGroupCode(r.getText(11));
			rItem.setLabel1(r.getText(22));
			rItem.setLabel2(r.getText(23));
			rItem.setLabel3(r.getText(24));
			rItem.setLabel4(r.getText(25));
			rItem.setLabel5(r.getText(26));
			rItem.setLabel6(r.getText(27));
			rItem.setLabel7(r.getText(28));
			rItem.setLabel8(r.getText(29));
			rItem.setLabelStr(r.getText(30));
			rItem.setDefaultLabel(r.getFlag(1) ? "1" : "0");
			rItem.setMutipleChoiceLabel(r.getFlag(2) ? "1" : "0");
	...
	public String getFilePath(String projectId, String fileName) {
		...
		String yearMonthDir = projectId.substring(0, 6);
		String fileDir = fileName.substring(fileName.indexOf("-"));
		fileDir = fileDir.substring(0, fileDir.lastIndexOf("-"));
		// 获取文件加路径
		return svnDir + Constants.SVN_DIR_PC + File.separator + yearMonthDir + File.separator + projectId + fileDir;
	}
}
